-- ═══════════════════════════════════════════════════════════════════
-- Devolver la frescura del contacto en el drawer
--
-- 20260824180000_drawer_contact_freshness.sql había agregado `created_at` y
-- `updated_at` del CONTACTO al payload del drawer. Es lo que la capa 1 usa para
-- resolver qué fila de `contacts` es el perfil vigente cuando la misma persona
-- está duplicada (canonicalizeSignals lee `contact.updated_at ?? created_at`).
--
-- 20260825132651 reescribió get_company_drawer_data entera desde la definición
-- que estaba viva en la base, y en esa definición los dos campos ya no estaban:
-- se perdieron en el camino. Verificado contra producción — el md5 del cuerpo de
-- la función coincide con el del archivo (8251cb20d2936091073eee084652879d) y
-- ninguno de los dos tiene `ct.updated_at`.
--
-- Sin ellos el drawer no deja de deduplicar personas —eso lo decide la identidad,
-- no la fecha— pero sí pierde el criterio para elegir de cuál de las dos filas
-- muestra el puesto y la empresa, y puede quedarse con la vieja.
--
-- Esta migración es la de 132651 con los dos campos devueltos a los DOS payloads
-- de contacto: el de señales y el de alumni.
-- ═══════════════════════════════════════════════════════════════════
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'contact', jsonb_build_object(
        ...
        -- Frescura del perfil: con dos filas en `contacts` de la misma
        -- persona, esto es lo que decide cuál se muestra.
