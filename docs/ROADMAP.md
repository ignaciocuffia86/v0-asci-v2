...
|---|-----------|-----------|-----------|
| V3-F | **Radiografia comercial self-service**: el bookmark genera solo el informe que hoy se arma a mano (movimientos de personal con fecha de ingreso, scorecard operativo con semaforo, vacantes con snippet segun propuesta, radar de noticias con lectura por vendor, angulos/riesgos, metodo autogenerado, export + digest). Fases 7-10; fases 7 (fechas de puesto + movimientos), 8 (noticias) y 9 (informe vertical) implementadas ago-2026. La busqueda de noticias corre el bundle caro (2 focos con haiku, ~US$0,20/cuenta, sin cupo ni confirmacion de costo), es unica para v2 y v3 (`lib/shared/news-search.ts`) y se dispara al marcar el bookmark, con refresco mensual por cron. Queda la fase 10 (export + digest) | Alta (norte del producto) | [Diseño, seccion F](./feature-v3-experiencia-cuentas-y-chat.md) |
