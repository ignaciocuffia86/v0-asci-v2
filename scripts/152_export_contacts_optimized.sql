-- export_contacts v4 - Optimized version
-- Fixes timeout issues by simplifying query and removing expensive operations
...
BEGIN
  ...
  SELECT
    ...
    s.signal_type::TEXT,
    COALESCE(dp.name, dprod.name)::TEXT     AS signal_name
  FROM signals s
  ...
  WHERE
    ...
    -- Signal names filter (case insensitive)
    AND (
    ...
    -- Country filter (using country_normalized with fallback)
    AND (
      p_countries IS NULL
