-- Fix: Alumni signals were hardcoded to 'technology' signal_type
-- This caused alumni to show 0 when searching by process
-- The fix uses p_filter_type parameter dynamically
...
BEGIN
  ...
  -- 5. Get job postings with signals (single query)
  ...
  FROM (
    SELECT DISTINCT ON (jp.id) jsonb_build_object(
      ...
      'keyword_matched', s.keyword_matched,
      'signal_name', COALESCE(dp.name, s.keyword_matched),
      'snippet', s.snippet
    ) as jp_data
    FROM signals s
  ...
  -- 6. Get alumni signals (contacts with company in previous_positions)
  -- Fix: Use p_filter_type instead of hardcoded 'technology'
  -- If p_filter_type is NULL, show all signal types
  ...
  FROM (
    ...
    -- Dynamic signal_type filter based on p_filter_type parameter
    AND (
