-- Script 025: Fix Empty LinkedIn URL Deduplication
-- This script updates process_import_batch to treat empty LinkedIn URLs as unique contacts
-- instead of merging them into a single record.
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Calculate LinkedIn URL (handle empty strings as unique placeholders)
      v_linkedin_url := v_row.row_data->>'linkedin_url';
      IF v_linkedin_url IS NULL OR TRIM(v_linkedin_url) = '' THEN
