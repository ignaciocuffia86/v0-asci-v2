-- Function to get all drawer data in a single query
-- This eliminates the N+1 problem by fetching all data with JOINs
...
-- Create index for previous_positions JSONB queries if not exists
CREATE INDEX IF NOT EXISTS idx_contacts_previous_positions_gin 
ON contacts USING GIN (previous_positions jsonb_path_ops);
