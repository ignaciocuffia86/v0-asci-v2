-- RPC v7: Export contacts directly from signals table
-- This is the source of truth - signals table contains all detected signals
-- We filter by: signal type/name, country, industry, and corporate email
...
CREATE OR REPLACE FUNCTION export_contacts(
  ...
  p_countries TEXT[] DEFAULT NULL,           -- Array of countries (from companies.country_normalized)
  p_industries TEXT[] DEFAULT NULL,          -- NEW: Array of industries to filter
...
BEGIN
  ...
  WHERE 
    ...
    -- Filter by countries
    AND (p_countries IS NULL OR comp.country_normalized = ANY(p_countries))
