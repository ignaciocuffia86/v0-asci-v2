-- Nueva RPC para búsqueda por tecnología con scoring y filtro de industria
-- El scoring combina: empleados actuales (40%), alumni (30%), búsquedas laborales (30%)

DROP FUNCTION IF EXISTS search_companies_by_technology_v2(uuid, text[], boolean);
...
BEGIN
  ...
  scored AS (
    SELECT 
      cs.*,
      -- Score individual por componente (0-100)
...
-- Nueva RPC para búsqueda por proceso con scoring y filtro de industria
...
BEGIN
  ...
  SELECT 
    GREATEST(MAX(cnt.current_employees), 1),
    GREATEST(MAX(cnt.alumni_count), 1),
    GREATEST(MAX(cnt.job_postings), 1)
  INTO max_current, max_alumni, max_job_postings
  FROM (
    SELECT 
  ...
  WITH company_signals AS (
    SELECT 
      ...
      COUNT(DISTINCT s.id) AS total_signals,
      COUNT(DISTINCT s.id) FILTER (WHERE s.is_current_employee = TRUE) AS current_employees,
      COUNT(DISTINCT s.id) FILTER (WHERE s.is_current_employee = FALSE) AS alumni_signals,
      (
        SELECT COUNT(DISTINCT jp.id)
        FROM job_postings jp
        INNER JOIN signals js ON js.job_posting_id = jp.id
