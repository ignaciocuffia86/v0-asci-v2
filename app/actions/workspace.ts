...
export async function generateSimplifiedIcebreaker(
  bookmarkId: string,
  contactId: string,
  companyName: string,
  isDM = false,
  ...
  if (isDM) {
    const { data } = await supabase.from("user_company_contacts").select("*").eq("id", contactId).single()
    contact = data
    if (contact) {
      // Normalize fields
      contact.current_position_title = contact.role
    }
  } else {
    const { data } = await supabase
      .from("contacts")
      .select("id, full_name, first_name, headline, current_position_title, linkedin_url")
      .eq("id", contactId)
      .single()
    contact = data
  }
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("company, value_proposition")
    .eq("id", user.id)
    .single()
  ...
  const userCompanyName = profile?.company || "nuestra empresa"
  const valueProposition = strategy?.sender_context_override || profile?.value_proposition || ""

  let dmContext = ""
  if (isDM) {
    // Get news and implementations for richer context
    const [newsResult, implementationsResult] = await Promise.all([
      supabase
        .from("company_news")
    ...
    if (news.length > 0 || implementations.length > 0) {
      dmContext = `

## Información relevante de la compañía:
${news.length > 0 ? `Noticias recientes: ${news.map((n) => n.title).join(", ")}` : ""}
${implementations.length > 0 ? `Implementaciones: ${implementations.map((i) => i.title).join(", ")}` : ""}
`
    }
  }

  const prompt = `Genera dos mensajes de prospección B2B para ${contact.first_name || contact.full_name?.split(" ")[0] || "el contacto"}:

## Contexto del destinatario:
- Nombre: ${contact.full_name}
- Cargo: ${contact.current_position_title || contact.headline || "No especificado"}
- Empresa: ${companyName}
${contact.headline ? `- Headline: ${contact.headline}` : ""}
${isDM ? "- Es un tomador de decisión identificado via Apollo.io" : ""}
${dmContext}

## Contexto del remitente:
- Empresa: ${userCompanyName}
${valueProposition ? `- Propuesta de valor: ${valueProposition}` : ""}
${strategy?.recommended_pitch ? `- Pitch recomendado: ${strategy.recommended_pitch}` : ""}

## Instrucciones:

1. **MENSAJE LINKEDIN** (máximo 300 caracteres):
- Mensaje de conexión corto y personalizado
- Mencionar algo específico del perfil del destinatario
- NO incluir links ni pedir reuniones directamente
- Terminar con una pregunta abierta sobre su trabajo

2. **EMAIL DE SEGUIMIENTO** (máximo 500 caracteres):
- Asumir que ya enviaste el mensaje de LinkedIn
- Referencia a tu mensaje anterior
- Mencionar cómo ${userCompanyName} puede ayudar
- Terminar con pregunta abierta, NO pedir reunión directamente

IMPORTANTE: 
- Escribe en español argentino (usar "vos" en lugar de "tú")
- NO uses formato markdown ni asteriscos
- NO incluyas saludos como "Espero que estés bien"
- Sé directo y profesional
- Los mensajes deben poder copiarse y pegarse directamente

Responde EXACTAMENTE con este formato:
---LINKEDIN---
[tu mensaje de linkedin aquí]
...
[tu email aquí]`
  ...
  const linkedinMessage = linkedinMatch?.[1]?.trim() || ""
  ...
  const baseRecord = {
    ...
    company_id: bookmark.company_id,
    bookmark_id: bookmarkId,
    contact_id: isDM ? null : contactId, // For DM, contact_id references user_company_contacts
    template_used: "simplified",
    context_used: `Contacto: ${contact.full_name}, Empresa: ${companyName}${isDM ? " (DM)" : ""}`,
  }
