...
const INSTRUCTIONS = [
  ...
  "EXCEPCIÓN 1 — Apollo. `run_contact_enrichment` gasta créditos de un tercero y ese gasto NO se recupera. En un lote la confirmación es UNA sola: `estimate_batch` muestra el total de créditos y su costo en dólares, y el batchPlanHash que el usuario acepta AUTORIZA ese gasto. Pasale el `batchJobId` a `prepare_contact_enrichment` y ya no vuelvas a preguntar: el techo pasa a ser el presupuesto del lote y cuando se agota, para. Fuera de un lote no hay autorización previa, así que ahí sí mostrá el costo y confirmá antes de ejecutar.",
