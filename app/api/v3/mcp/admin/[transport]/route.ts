...
const INSTRUCTIONS = [
  ...
  "TELÉFONOS: van por `request_contact_phones`, aparte del enrichment. Cuestan 5 créditos contra 1 del email, por eso la tool pide solo para los contactos con email verificado y cargo que matchea. NO ESPERA: Apollo entrega por webhook y el 57% de las veces llega, así que la tool te dice qué pidió y los números se leen después con `get_company_contacts`. No anuncies un teléfono que no leíste ahí. Y mirá `skipped`: los que ya tenían número no se volvieron a pedir, y `creditsSaved` dice cuánto no se gastó por eso.",
