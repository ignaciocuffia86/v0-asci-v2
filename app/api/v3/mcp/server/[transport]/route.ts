...
const handler = createMcpHandler((rawServer) => {
  ...
  server.tool("search_companies_by_capability", "Búsqueda INVERSA: qué EMPRESAS usan una tecnología o ejecutan un proceso. Es la tool para \"qué bancos usan Dynamics 365\" o \"qué empresas de retail tienen SAP\". Antes esto no se podía y el asistente respondía que ASCI no permitía búsquedas inversas, cuando los datos SÍ están. Solo lectura, sin IA y sin consumir cupo. CIRCUITO DE DOS PASOS: llamá primero con mode=\"screening\", que devuelve SOLO conteos por país e industria (sin nombres); si son muchas, preguntale al usuario por qué país o industria empezar y recién entonces pedí mode=\"detail\". No pidas detail sin filtros para un término amplio: son cientos de empresas que no aportan nada. Los países se pasan con el nombre EXACTO que devolvió el screening (\"Argentina\"), no con código ISO. Un término puede resolver a varias entradas del diccionario (\"Dynamics 365\" son CRM y ERP; \"Microsoft\" son 10 productos) y el resultado las SUMA: mirá resolvedTerms para saber qué se buscó y matchedTerms de cada empresa para saber cuál usa. Los procesos son categorías muy amplias: máximo 2 por llamada. Leé siempre guidance, que indica el próximo paso.", { terms: z.array(z.string().min(2)).min(1).max(10), countries: z.array(z.string().min(2)).max(10).optional(), industries: z.array(z.string().min(2)).max(10).optional(), includeProviders: z.boolean().default(false), mode: z.enum(["screening", "detail"]).default("screening"), limit: z.number().int().min(1).max(50).default(25) }, async ({ terms, countries, industries, includeProviders, mode, limit }, extra) => safely(async () => { const auth = authOf(extra); await requirePaidMcp(auth, "companies:read", "read"); return searchCompaniesByCapability({ terms, countries, industries, includeProviders, mode, limit }) }))
