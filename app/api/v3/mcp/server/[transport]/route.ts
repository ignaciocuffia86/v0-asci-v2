...
const handler = createMcpHandler((rawServer) => {
  ...
  server.tool("get_company_signal_summary", "Resume evidencia v2 de una empresa consolidando aliases relacionados: señales tecnológicas y de procesos, implementaciones y vacantes. Cada evidencia conserva su entidad de origen; las vacantes sin señales vinculadas son indicios, no confirmaciones. Usá esta tool cuando el usuario pregunte qué tecnologías, procesos o señales tenemos de una empresa.\n\nELEGÍ BIEN EL `detail`:\n• \"compact\" (default) — devuelve [{label, type, count, lastSeen}] por término, más conteos de implementaciones y vacantes, SIN fragmentos de texto. Es la que tenés que usar para VALIDAR VARIAS CUENTAS de una búsqueda: cuesta una fracción y alcanza para descartar (se ve enseguida si el término que buscabas no está entre las tecnologías principales de la cuenta).\n• \"full\" — suma hasta 3 fragmentos de evidencia por término, las implementaciones completas y hasta 30 vacantes con su descripción. Son varios miles de tokens por cuenta: pedila para UNA cuenta que ya decidiste mirar en serio, no para revisar una lista.\n\nSi lo que necesitás es de dónde sale UN término concreto (fragmento exacto, fecha, link y de qué persona se infiere), no pidas \"full\": usá get_account_evidence_detail.", { companyId: z.string().uuid(), detail: z.enum(["compact", "full"]).default("compact").describe("\"compact\" para revisar varias cuentas; \"full\" solo para una cuenta que vas a trabajar.") }, async ({ companyId, detail }, extra) => safely(async () => { const auth = authOf(extra); await requirePaidMcp(auth, "signals:read", "read"); return getCompanySignalSummary(companyId, detail) }))
