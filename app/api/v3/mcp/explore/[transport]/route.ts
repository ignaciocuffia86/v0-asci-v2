...
const handler = createMcpHandler(
  (rawServer) => {
    ...
    server.tool(
      "explore_companies",
      "Ranking de empresas por concepto, con filtros explícitos y SIN necesitar sesión (para re-consultas o ajustes rápidos). Pasá `terms` (nube expandida) y opcionalmente country, industryIds, includeUnclassified y sources. Devuelve lo mismo que explore_set_industries: empresas con personMatches (gente que sabe) y jobMatches (vacantes que lo piden, en todo el histórico y sin que is_active signifique nada: viene en true en las 43.052 del catálogo). Usá explore_start si preferís el embudo guiado paso a paso.",
      {
