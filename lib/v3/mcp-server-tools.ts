...
import { requestContactPhones } from "@/lib/v3/services/mcp-contact-phones"
...
import { getCostSummary } from "@/lib/v3/services/mcp-cost-summary"
...
// ═══════════════════════════════════════════════════════════════════════════
// LOS DOS PERFILES DEL MCP v3, sobre UNA sola registración de tools.
//
// `standard` es el cliente: los topes de cuenta y cupo lo frenan, y cada
// operación que gasta pide confirmación. `admin` es el equipo de ASCI armando un
// informe on-demand: sin topes (ver lib/v3/admin-workspace.ts) y sin preguntar 42
// veces.
//
// POR QUÉ UN PROXY Y NO DOS ARCHIVOS. Duplicar las 44 registraciones para cambiar
// nueve frases es exactamente el bug que ya nos pasó con los scopes: dos copias,
// una se actualiza, la otra queda vieja y nadie se entera. Acá hay una sola
// fuente y una lista corta de diferencias declaradas.
//
// POR QUÉ REEMPLAZO DE FRASE Y NO DESCRIPCIÓN COMPLETA. Una descripción admin
// escrita aparte también se desincroniza en silencio. Una regla `find`/`replace`
// no puede: si alguien reescribe la descripción standard y la frase desaparece,
// la regla deja de aplicar y el test falla. El desfasaje se vuelve ruidoso.
// ═══════════════════════════════════════════════════════════════════════════

export type McpProfile = "standard" | "admin"

export type DescriptionRule = { tool: string; find: string; replace: string }

/**
 * Las diferencias de prompt entre `standard` y `admin`, declaradas una por una.
 *
 * Se tocan DOS cosas y ninguna más:
 *
 *  1. "la cuenta tiene que estar guardada" — en admin es falso: los guards no lo
 *     exigen, así que dejarlo escrito haría que el modelo pida guardar cuentas
 *     que no hace falta guardar, ocupando el workspace con cientos de filas.
 *  2. "pedí confirmación porque consume cupo" — en admin no hay cupo que
 *     consumir. La confirmación por CUOTA desaparece.
 *
 * Lo que NO se toca, y es deliberado: `run_contact_enrichment` y
 * `prepare_contact_enrichment` conservan su confirmación entera. El crédito de
 * Apollo es irreversible y eso no depende del perfil. `remove_workspace_account`
 * también la conserva: es destructivo.
 */
export const ADMIN_DESCRIPTION_RULES: DescriptionRule[] = [
  {
    tool: "recommend_contact_roles",
    find: "Requiere que la cuenta esté guardada.",
    replace: "No hace falta que la cuenta esté guardada: este perfil trabaja sobre el catálogo global.",
  },
  {
    tool: "prepare_account_icebreaker",
    find: "Requiere que la cuenta esté guardada en el workspace.",
    replace: "No hace falta que la cuenta esté guardada en el workspace.",
  },
  {
    tool: "run_account_research",
    find: "Requiere que TODAS las cuentas del lote estén guardadas en el workspace: si alguna no lo está, devuelve el detalle sin consumir cuota y hay que llamar save_account antes de reintentar.",
    replace: "No exige que las cuentas estén guardadas: este perfil investiga sobre el catálogo global.",
  },
  {
    tool: "prepare_account_research",
    find: "Requiere que todas las cuentas estén guardadas en el workspace: si alguna no lo está, devuelve el detalle sin consumir cuota y hay que llamar save_account antes de reintentar.",
    replace: "No exige que las cuentas estén guardadas en el workspace.",
  },
  {
    tool: "scrape_company_job_postings",
    find: "Trae vacantes frescas de LinkedIn para una cuenta guardada",
    replace: "Trae vacantes frescas de LinkedIn para cualquier empresa del catálogo",
  },
  {
    tool: "scrape_company_job_postings",
    find: "Consume cuota de research server-managed porque el scraping corre con recursos de ASCI.",
    replace: "El scraping corre con recursos de ASCI y su consumo se registra: cerrá el informe con get_cost_summary. No pidas confirmación para cada scraping.",
  },
  {
    tool: "get_company_contacts",
    find: "para una cuenta guardada",
    replace: "para una cuenta del catálogo",
  },
  {
    tool: "save_account",
    find: "Requiere confirmación explícita del usuario porque consume cupo.",
    replace: "En este perfil no hay cupo de cuentas: guardá sin preguntar cuando el lote lo necesite.",
  },
  {
    tool: "prepare_save_account",
    find: "Llamala siempre antes de save_account para poder pedir confirmación al usuario.",
    replace: "En este perfil no hace falta llamarla antes de save_account: no hay cupo que confirmar.",
  },
]

/**
 * Reescribe las descripciones según el perfil, sin tocar las registraciones.
 *
 * Mismo mecanismo que `withToolAudit`, que ya intercepta `server.tool` para
 * auditar: acá se interviene el argumento de la descripción. Devuelve `S` para
 * no perder la inferencia de los esquemas zod.
 *
 * `applied` cuenta cuántas veces se aplicó cada regla, y es lo que el test usa
 * para detectar una regla que dejó de encontrar su frase.
 */
export function withProfileDescriptions<S extends object>(
  server: S,
  rules: DescriptionRule[],
  applied?: Map<string, number>,
): S {
  if (!rules.length) return server
  return new Proxy(server, {
    get(target, prop, receiver) {
      const original = Reflect.get(target, prop, receiver)
      if (prop !== "tool" || typeof original !== "function") return original
      return (...args: unknown[]) => {
        ...
        if (typeof toolName === "string" && typeof args[1] === "string") {
          let description = args[1]
          for (const rule of rules) {
            if (rule.tool !== toolName || !description.includes(rule.find)) continue
            description = description.split(rule.find).join(rule.replace)
            const key = `${rule.tool}::${rule.find}`
...
export function registerV3Tools(
  ...
  server.tool("prepare_contact_enrichment", "Previsualiza una búsqueda de tomadores de decisión en Apollo SIN gastar créditos. Devuelve los cargos validados, cuántos contactos como máximo, el costo en créditos y el cupo mensual restante. MIRÁ `droppedRoles`: si trae algo, son cargos VÁLIDOS que no entraron por el tope del plan y esa búsqueda va a ser más angosta que la que pediste — decíselo al usuario antes de ejecutar, no después. Llamá antes a get_company_contacts: si la cobertura ya alcanza, no hace falta gastar. Devuelve un planHash que DEBÉS mostrar al usuario junto al costo y confirmar explícitamente antes de llamar a run_contact_enrichment. Salvo que pases `batchJobId`: ahí el gasto ya está autorizado por el batchPlanHash que el usuario confirmó, y el campo `ceiling` de la respuesta te dice cuál de los dos techos rige y cuánto queda. Si likelyCacheHit es true, ejecutar no gasta créditos. IMPORTANTE: si resolvedOrganization trae un warning, la identidad de la empresa se resolvió por nombre (match difuso) — mostrale al usuario el nombre, dominio y tamaño que devolvió Apollo y confirmá que es la empresa correcta (no una fusionada, renombrada u homónima) antes de ejecutar.", { companyId: z.string().uuid(), roles: z.array(z.string().min(2).max(120)).min(1).max(25), maxContacts: z.number().int().min(1).max(50).optional(), useOrganizationLocation: z.boolean().optional(), idempotencyKey: z.string().min(8).max(200).optional(), batchJobId: z.string().uuid().optional().describe("Lote que AUTORIZA este gasto (el id de create_batch_job). Con él, el techo pasa a ser el presupuesto de ese lote en vez del cupo mensual del plan, y no hace falta volver a confirmar: la confirmación ya la dio quien aceptó el batchPlanHash. La empresa tiene que estar en el lote.") }, async (args, extra) => safely(async () => { const auth = authOf(extra); return prepareContactEnrichment(auth, args) }))
  server.tool("run_contact_enrichment", "Ejecuta el enrichment de contactos en Apollo y GASTA CRÉDITOS. Solo acepta un planHash devuelto por prepare_contact_enrichment, y ese plan congela los cargos y el máximo de contactos: no podés cambiarlos acá.\n\nCUÁNDO HAY QUE CONFIRMAR, que depende de cómo se preparó el plan y la respuesta te lo dice en `authorizedBy`:\n• `authorizedBy.source = \"batch\"` — el plan se preparó con un batchJobId y el gasto YA ESTÁ AUTORIZADO por el batchPlanHash que el usuario aceptó al cotizar el lote. NO vuelvas a pedir confirmación cuenta por cuenta: esa autorización ya se dio una vez para el lote entero, y el techo es el presupuesto del lote. Cuando se agota, la preparación falla sola.\n• `authorizedBy.source = \"monthly\"` — el plan se preparó suelto, sin lote. Ahí NO hay autorización previa: mostrale al usuario el costo de la preparación y confirmá antes de ejecutar.\n\nEs idempotente: reejecutar el mismo planHash devuelve el resultado guardado sin volver a cobrar. Solo obtiene emails; el teléfono no se pide en esta tool.\n\nDOS NÚMEROS, NO UNO: `creditsSpent` es lo que se pagó y `creditsSaved` son los contactos que ya estaban en la base y no hubo que volver a pedir. Reportá los dos. Sin el segundo, un `creditsSpent: 0` con 8 contactos se lee como si Apollo los hubiera regalado, cuando lo que pasó es que ya los teníamos.", { planHash: z.string().min(16).max(64), userConfirmed: z.literal(true) }, async ({ planHash }, extra) => safely(async () => { const auth = authOf(extra); return runContactEnrichment(auth, { planHash }) }))

  server.tool("request_contact_phones", "Pide a Apollo el teléfono de los contactos de UNA cuenta, y GASTA CRÉDITOS: 5 por pedido, contra 1 que cuesta un email.\n\nNO ESPERA, y no es una limitación sino el diseño. El reveal de Apollo es asíncrono y entrega el 57% de las veces (medido sobre 141 pedidos). Esta tool pide, te dice QUÉ pidió, y termina. Los teléfonos NO vienen en la respuesta: se leen después con `get_company_contacts`, en `hasPhone` y `phoneStatus`.\n\nA QUIÉN LE PIDE: solo a los contactos con EMAIL VERIFICADO y CARGO QUE MATCHEA. Es deliberado — a 5 créditos cada uno, se paga solo por los que ya probaron ser los correctos.\n\nA QUIÉN NO, y mirá `skipped` porque es la mitad de la respuesta: al que YA TIENE el número (está en el caché compartido; no se paga dos veces por el mismo dato) y al que tiene un PEDIDO EN CURSO. `creditsSaved` cuenta lo que NO se gastó por eso.\n\nReportá los dos números, `creditsSpent` y `creditsSaved`, y no anuncies teléfonos hasta haberlos leído con get_company_contacts.", { companyId: z.string().uuid(), maxContacts: z.number().int().min(1).max(50).optional().describe("Tope de pedidos de ESTA llamada (default 50). No es un cupo del plan: es cuántos créditos gasta esta llamada."), userConfirmed: z.literal(true) }, async ({ companyId, maxContacts }, extra) => safely(async () => { const auth = authOf(extra); return requestContactPhones(auth, { companyId, maxContacts }) }))
  ...
  server.tool("run_account_research", "Lanza research server-managed con AI Gateway de ASCI. Consume cuota y hard limits; operación atómica por lote. PASÁ EL companyId (UUID) que devolvió search_companies o save_account, no el nombre: con un UUID la cuenta queda identificada sin ambigüedad, mientras que un nombre se resuelve por match difuso y puede apuntar a un homónimo distinto del que guardaste. Requiere que TODAS las cuentas del lote estén guardadas en el workspace: si alguna no lo está, devuelve el detalle sin consumir cuota y hay que llamar save_account antes de reintentar.", { companies: z.array(z.string().min(2)).min(1).max(10).describe("companyId (UUID) de cada cuenta. Se acepta el nombre solo si no tenés el id, pero es ambiguo."), forceRefresh: z.boolean().default(false), idempotencyKey: z.string().min(8).max(200) }, async ({ companies, forceRefresh, idempotencyKey }, extra) => safely(async () => {
    ...
    const reservation = await reserveMcpUsage({ principal: auth, pool: "research_server", units: canonical.length, idempotencyKey, metadata: { kind: "research", companies: canonical } })
  ...
  server.tool("scrape_company_job_postings", "Trae vacantes frescas de LinkedIn para una cuenta guardada y las ingesta por el pipeline de importación de ASCI, que las normaliza y deduplica. El filtro por empresa se aplica en el buscador usando el nombre y el país que ya tenemos guardados de la cuenta, así que no hace falta pasarlos. Consume cuota de research server-managed porque el scraping corre con recursos de ASCI. Dos cosas a tener en cuenta: (1) `titleQuery` es OPCIONAL y sirve para acotar por título de puesto DENTRO de la empresa (por ejemplo 'SAP'); si lo omitís se traen todas las vacantes de la empresa, que es lo habitual. (2) La ventana sólo puede ser 1, 7 o 30 días; si pedís más, se busca sin límite de fecha y se te avisa. ESTA es la forma de averiguar qué posiciones abiertas tiene una cuenta: no busques vacantes en la web, porque los resultados web no entran al pipeline de ASCI y no se pueden atribuir a la cuenta. Devuelve `preview` con las vacantes ya filtradas (título, ubicación y URL) para que puedas responder en el mismo turno; la ingesta normalizada es asincrónica y se completa después.", { companyId: z.string().uuid(), titleQuery: z.string().min(2).max(120).optional(), location: z.string().min(2).max(120).optional(), windowDays: z.number().int().min(1).max(365).optional(), maxRows: z.number().int().min(1).max(200).optional(), idempotencyKey: z.string().min(8).max(200), batchJobId: z.string().uuid().optional().describe("Si este scraping es parte de un lote, el id que devolvió create_batch_job. Solo sirve para atribuirle el costo a ese informe en get_cost_summary; no cambia lo que se scrapea.") }, async ({ companyId, titleQuery, location, windowDays, maxRows, idempotencyKey, batchJobId }, extra) => safely(async () => {
    ...
    if (blocked) return blocked
    const reservation = await reserveMcpUsage({ principal: auth, pool: "research_server", units: 1, idempotencyKey, metadata: { kind: "job_scrape", companyId, titleQuery: titleQuery ?? null, batchJobId: batchJobId ?? null } })
    if (!reservation.allowed || !reservation.reservationId) return reservation
...
export function createV3McpHandler(options: { profile: McpProfile; basePath: string; instructions: string }) {
  const handler = createMcpHandler((rawServer) => registerV3Tools(rawServer, options.profile), {
    serverInfo: { name: options.profile === "admin" ? "asci-v3-admin" : "asci-v3", version: "2.0.0" },
    instructions: options.instructions,
  }, { basePath: options.basePath, maxDuration: 120, verboseLogs: false })

  return withMcpAuth(handler, async (req: Request, token?: string) => {
    ...
    await logMcpRequest({ principal, method: req.method, statusCode: 200, requestId: crypto.randomUUID(), metadata: { phase: "transport", profile: options.profile } })
